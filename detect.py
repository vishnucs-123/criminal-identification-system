...
import winsound
import cv2
# This function loads and ensures the image is in RGB format
def load_image(image_path):
    img = Image.open(image_path)
    
    # Check if the image has an alpha channel (RGBA) or grayscale, then convert it to RGB
    if img.mode != 'RGB':
        img = img.convert('RGB')
...
def View():
    cleartree()
    frame = cv2.imread("C:/Users/VAISHNAVI/Face-identificaton/temp/1.png")
...
images_name = []
for img in images:
      images_name.append(load_image(os.path.join("images", img)))
